...
import { Card, Row, Col, Statistic, List, Tag, Spin, Typography, Progress } from 'antd';
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={12} md={6}>
          <Card>
            <Statistic
              title="业绩目标"
              value={revenueTarget ? `${Math.round((revenueTarget.progress || 0) / (revenueTarget.target_value || 1) * 100)}%` : '未设定'}
              prefix={<AimOutlined />}
          ...
          <Card title="目标进度" size="small">
            {revenueTarget ? (
              <div style={{ marginBottom: 16 }}>
                <Typography.Text>业绩目标</Typography.Text>
                <Progress
                  percent={Math.round((revenueTarget.progress || 0) / (revenueTarget.target_value || 1) * 100)}
                  format={() => `${revenueTarget.progress || 0} / ${revenueTarget.target_value}`}
                />
              </div>
            ) : null}
            {contactTarget ? (
              <div>
                <Typography.Text>触达目标</Typography.Text>
                <Progress
                  percent={Math.round((contactTarget.progress || 0) / (contactTarget.target_value || 1) * 100)}
                  format={() => `${contactTarget.progress || 0} / ${contactTarget.target_value}`}
                />
              </div>
            ) : null}
