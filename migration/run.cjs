...
function fetchAll(tableId) {
  ...
  let pt = null;
  do {
    let cmd = `lark-cli base +record-list --base-token "${BASE}" --table-id "${tableId}" --as user --limit 100`;
...
async function main() {
  ...
  for (const r of customers) {
    ...
    map.customers[r.record_id] = map.customers[r.record_id] || (await prisma.customer.findFirst({ where: { lark_record_id: r.record_id }, select: { id: true } }))?.id;
  }
  // Reload mapping
